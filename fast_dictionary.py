...
class FastDictionary(plugins.Plugin):
    ...
    __version__ = "1.0.0"
    __license__ = "GPL3"
    __description__ = "Run a fast dictionary scan against captured handshakes. Based on rossmarks.uk plugin FAST_DICTIONARY.py"
    __dependencies__ = {
        "apt": ["aircrack-ng"],
        "pip": ["scapy"],
    }
    ...
    def install_aircrack(self):
        ...
        self.generate_log("Installing aircrack-ng")
        subprocess.run(["apt", "install", "aircrack-ng", "-y"])
        self.generate_log("aircrack-ng installed")

    def on_loaded(self):
        self.generate_log("Plugin loaded.")
        ...
        if check_aircrack_installation.returncode == 0:
            self.generate_log("aircrack-ng is installed. Waiting handshakes to crack!")
            self.aircrack = check_aircrack_installation.stdout.strip()
            self.needs_install = False
        else:
            self.generate_log("aircrack-ng is not installed!", type="WARNING")
            self.needs_install = True

    def find_ssid_and_bssid(self, filename):
        ...
        try:
            ...
            self.generate_log(f"Error finding ssid or bssid on pcap: {e}", type="ERROR")
            return [], []
    ...
    def crack_handshake(self, filename, bssid_or_ssid, type="SSID"):
        ...
        try:
            ...
            for wordlist in list_of_wordlists:
                ...
                self.generate_log(command, type="DEBUG")
    ...
    def has_cracked(self, cracked_file):
        ...
        try:
            ...
            self.generate_log(f"Error verifying cracking: {e}", type="ERROR")
            return False

    def on_internet_available(self, agent):
        """Install aircrack-ng if needed and internet is available"""
        if self.needs_install:
            self.generate_log("Internet available. Installing aircrack-ng")
        ...
        try:
            ...
            for packet in packets:
                if packet.haslayer("EAPOL"):
                    ...
                    self.generate_log(f"Handshake found on {filename}", type="DEBUG")
                    has_eapol = True
                    break
            if not has_eapol:
                self.generate_log(f"No handshake, deleting pcap file {filename}")
                self.delete_file(agent, filename)
                return
            else:
                self.generate_log(
            ...
            self.generate_log(f"Unexpected error: {e}", type="ERROR")
